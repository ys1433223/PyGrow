...
- problems 列出2-4个具体问题，问题必须紧扣项目要求，指出哪些需求未被满足或实现不正确
- suggestions 给出2-4条具体修改建议，建议必须围绕如何满足项目要求展开
- related_knowledge 列出2-4个需要复习的知识点
- 评价要客观、鼓励，适合初学者
- 如果是空提交或明显无效提交（完全没有代码或内容），total_score为0，level为"需修改"
- **核心原则：严格根据项目要求评分，学生提交了无关但可运行的代码不能得分。不被学生自述说明带偏。**
"""
...
{student_work}
重要批改规则：
- **以项目要求为核心**：问题和修改建议必须紧扣「项目要求」中的具体需求来评判。学生提交了无关但可运行的代码不能算作正确。
- **功能完整性是第一标准**：逐一对照项目要求，检查每个功能点是否被正确实现。
- **若学生提交的代码与项目要求无关**：直接扣分并指出哪些要求未被实现，不要被无关代码带偏。
- **代码说明不作为评分依据**：只依据代码本身评判，不参考学生自述的项目说明。
